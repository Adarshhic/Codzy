...
export const initializeStreamClients = async (userId, userName, token) => {
  try {
    ...
    if (!apiKey) {
      throw new Error('VITE_STREAM_API_KEY is not set in environment variables. Please add it to your .env file.');
    }

    console.log('Initializing Stream clients with:', { userId, userName, hasToken: !!token, hasApiKey: !!apiKey });
